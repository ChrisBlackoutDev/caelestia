...
The pending-upgrade list must be empty, NetworkManager must match `libnm`, and installed GCC runtime packages must match. Also verify free space, package locks, boot artifacts, network/SSH continuity, the recovery path, and current package ownership.
